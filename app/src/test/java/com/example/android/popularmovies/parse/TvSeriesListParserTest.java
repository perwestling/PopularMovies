...
public class TvSeriesListParserTest {
    ...
    public void shouldThrowIfMissingName() throws Exception {
        try {
            TvSeriesListParser.parse(makeTvSeriesListJson(TV_SERIES_1.replace("\"name\":\"SWAT Kats: The Radical Squadron\",", "")));
            fail("Should have thrown");
        } catch (ParseException e) {
            assertThat(e.getMessage(), containsString("Failed to parse name"));
        }
    }
    ...
    public void shouldParseNameOfOneTvSeries() throws Exception {
        ...
        assertThat(tv.title, is("SWAT Kats: The Radical Squadron"));
    }
